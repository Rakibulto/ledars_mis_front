...
export function CreateDirectPurchase() {
  ...
  const { data: formOptionsData } = useGetRequest(
    `${endpoints.procurement_management.direct_purchases}form-options/`
  );
  ...
  const recalcExtended = (index, qty, price) => {
    setValue(
      `dp_items.${index}.extended_amount`,
    ...
    handleSubmit(async (vals) => {
      ...
      try {
        ...
        [
          ...
          if (vals[f]) fd.append(f, vals[f]);
        });
        const dpItems = (vals.dp_items ?? [])
        ...
        if (isEditMode) {
          await axiosInstance.patch(
            `${endpoints.procurement_management.direct_purchases}${editId}/`,
            fd
          );
        } else {
        ...
        setSubmitError(
          typeof errData === 'object'
            ...
            : String(errData ?? err.message ?? 'Network error')
        );
        setSubmitting(false);
      }
    });
          ...
          <div className="flex items-center justify-between gap-2 overflow-x-auto pb-1">
            {SECTIONS.map((section, idx) => (
              ...
              >
                ...
                <span
                  className={`max-w-24 text-center text-[10px] font-medium leading-tight ${idx === activeSection ? 'text-primary' : 'text-muted-foreground'}`}
                >
                  {section}
                </span>
              </button>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <div className="col-span-12 space-y-6 lg:col-span-9">
          ...
          {activeSection === 0 && (
            <Card>
              <CardHeader
                ...
                description="Department, project, priority and purpose"
                action={
              ...
              <CardBody>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full rounded-lg border border-input px-3 py-[9px] text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                      >
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-medium text-foreground">
                      Purpose / Justification Summary *
                    </label>
                    <textarea
                      {...register('purpose')}
                      rows={3}
                      placeholder="Brief purpose of this direct purchase…"
                      ...
                      <tbody>
                        {fields.map((field, index) => (
                          <tr key={field.id} className="border-b border-border">
                                ...
                                render={({ field: f }) => (
                                  <Autocomplete
                                    ...
                                    onChange={(_, val) => {
                                      ...
                                      if (val) {
                                        setValue(
                                          `dp_items.${index}.description`,
                                          val.item_name ?? val.name ?? ''
                                        );
                                        setValue(
                                          `dp_items.${index}.specification`,
                                          val.subcategory_name ?? val.specifications ?? ''
                                        );
                                        setValue(
                                          `dp_items.${index}.unit`,
                                          val.uom_name ?? val.unit ?? 'Pcs'
                                        );
                                        const rate =
                                          parseFloat(val.unit_price ?? val.cost ?? 0) || 0;
                                        setValue(`dp_items.${index}.estimated_rate`, rate);
                                        const qty =
                                          parseFloat(getValues(`dp_items.${index}.quantity`)) || 1;
                            ...
                            <td className="px-3 py-2">
                              <input
                                {...register(`dp_items.${index}.description`)}
                                type="text"
                            ...
                            <td className="px-3 py-2">
                              <input
                                {...register(`dp_items.${index}.specification`)}
                                type="text"
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      recalcExtended(
                                        index,
                                        qty,
                                        getValues(`dp_items.${index}.estimated_rate`)
                                      );
                                    }}
                                    type="number"
                                    min="1"
                                ...
                                render={({ field: f }) => (
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      recalcExtended(
                                        index,
                                        getValues(`dp_items.${index}.quantity`),
                                        rate
                                      );
                                    }}
                                    type="number"
                            ...
                            <td className="px-3 py-2">
                              <input
                                {...register(`dp_items.${index}.remarks`)}
                                type="text"
                                placeholder="Notes..."
                                className="w-full rounded border border-input px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
                              />
          ...
          {activeSection === 2 && (
            <Card>
              <CardHeader
                ...
                description="Technical specs, brand preferences, warranty and standards"
              />
              <CardBody>
                <div className="space-y-4">
                  <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Other acceptable brands"
                        className="w-full rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      placeholder="ISO, BDS, or other applicable standards…"
                      className="w-full rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary resize-none"
                    />
                  </div>
                </div>
          ...
          {activeSection === 3 && (
            <Card>
              <CardHeader
                ...
                description="Shop / Seller, delivery location, dates, payment terms and contact details"
              />
              <CardBody>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      placeholder="Packing, labelling, handling…"
                      className="w-full rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary resize-none"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="Why is this a direct purchase (sole source, urgency, etc.)…"
                      className="w-full rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary resize-none"
                    />
                  </div>
                  <div>
                    <label className="mb-2 block text-sm font-medium text-foreground">
                      Attachment (Quotation / Invoice)
                    </label>
                    <input
                      type="file"
                      ref={fileInputRef}
          ...
          {activeSection === 4 && (
            <Card>
              <CardHeader
                title="Review & Submit"
                description="Confirm all details before submission"
                action={
                ...
                <div className="space-y-4">
                    ...
                    <div className="rounded-lg border border-border p-4 space-y-2">
                      ...
                      <dl className="space-y-1 text-sm">
                        <div className="flex justify-between">
                          ...
                          <dd className="font-medium">{values.department?.name || '—'}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="text-muted-foreground">Project</dt>
                          <dd className="font-medium">{values.project?.name || '—'}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="text-muted-foreground">Priority</dt>
                          <dd className="font-medium">{values.priority?.label || '—'}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="text-muted-foreground">Fiscal Year</dt>
                          <dd className="font-medium">{values.fiscal_year || '—'}</dd>
                        </div>
                      </dl>
                    </div>
                    <div className="rounded-lg border border-border p-4 space-y-2">
                      ...
                      <dl className="space-y-1 text-sm">
                        <div className="flex justify-between">
                          <dt className="text-muted-foreground">Shop / Seller</dt>
                          <dd className="font-medium">
                            {typeof values.shop === 'string'
                              ? values.shop
                              : values.shop?.name || '—'}
                          </dd>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <dd className="font-medium">{values.payment_terms || '—'}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="text-muted-foreground">Purchase Date</dt>
                          <dd className="font-medium">{values.purchase_date || '—'}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="text-muted-foreground">Delivery By</dt>
                          <dd className="font-medium">{values.expected_delivery_date || '—'}</dd>
                        </div>
                        <div className="flex justify-between">
                          <dt className="text-muted-foreground">Contact</dt>
                          <dd className="font-medium">{values.contact_person || '—'}</dd>
                        </div>
                      </dl>
                    </div>
                  </div>
                  <div className="rounded-lg border border-border overflow-hidden">
                    <div className="bg-muted/50 px-4 py-2.5 flex items-center justify-between">
                      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                        BOQ ({watchedItems.filter((i) => i.description || i.item?.id).length} items)
                      </p>
                        ...
                        .map((item, idx) => (
                          ...
                          >
                            <span className="flex-1 text-foreground">
                              {item.description || item.item?.item_name || item.item?.name || '—'}
                            </span>
                            <span className="text-muted-foreground mx-4">
                              {item.quantity} × {fmtAmount(item.estimated_rate)}
                            </span>
                    ...
                    <div className="rounded-lg border border-border p-4">
                      ...
                      {selectedStatus !== editDpData?.status && (
                        <p className="text-xs text-amber-600 mt-2">
                          Status will change:{' '}
                          <span className="font-semibold">{editDpData?.status}</span> →{' '}
                  ...
                  <div className="flex items-center justify-end gap-3 pt-2">
                    ...
                    <Button
                      type="button"
                      variant="primary"
                      onClick={submitForm(isEditMode ? selectedStatus : 'Approved')}
                      disabled={submitting}
                    >
                      <Send className="h-4 w-4 mr-2" />
                      {submitting ? 'Saving…' : isEditMode ? 'Save Changes' : 'Submit DP'}
                    </Button>
                  </div>
                </div>
              </CardBody>
        ...
        <div className="col-span-12 space-y-4 lg:col-span-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span className="font-medium text-foreground">
                    {watchedItems.filter((i) => i.description || i.item?.id).length}
                  </span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                ...
                {values.shop && (
                  <div className="pt-2 border-t border-border">
                    ...
                    <p className="font-medium">
                      {typeof values.shop === 'string' ? values.shop : values.shop?.name}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col gap-2">
            {activeSection > 0 && (
              <Button variant="outline" size="sm" onClick={() => setActiveSection((v) => v - 1)}>
                ← Previous
              </Button>
            )}
            {activeSection < SECTIONS.length - 1 && (
              <Button variant="primary" size="sm" onClick={() => setActiveSection((v) => v + 1)}>
                Next →
              </Button>
            )}
          </div>
