...
export function DirectPurchaseList() {
  ...
  const apiUrl = useMemo(
    () =>
      buildUrl(endpoints.procurement_management.direct_purchases, {
        search,
        dp_number: dpNoFilter,
        status: statusFilter,
  ...
  const { data: statsData } = useGetRequest(
    `${endpoints.procurement_management.direct_purchases}summary/`
  );
  ...
  const statusBadge = (st) => {
    ...
    const Icon = cfg.icon;
    return (
      <Badge variant={cfg.variant} size="sm" className={`whitespace-nowrap ${cfg.className ?? ''}`}>
  ...
  const priorityBadge = (p) => (
    <span
      className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-semibold ${PRIORITY_BADGE[p] ?? 'bg-gray-100 text-gray-600 border border-gray-200'}`}
    >
      {p || '—'}
    </span>
  );
        ...
        <CardBody>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              {
                label: 'Converted to GRN',
                count: statsData?.converted_to_grn ?? 0,
        ...
        <CardBody>
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-[220px]">
              ...
              <input
                type="text"
                placeholder="Search by DP no, shop, department…"
                value={searchInput}
                  ...
                  {rows.map((dp, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <Link
                          href={paths.dashboard.procurement.directPurchase.detail(dp.id)}
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">
                          {dp.item_count != null
                            ? `${dp.item_count} item${dp.item_count !== 1 ? 's' : ''}`
                            : ''}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right font-semibold whitespace-nowrap">
                        {fmtAmount(dp.total_amount)}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                        {fmtDate(dp.purchase_date)}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                        {fmtDate(dp.expected_delivery_date)}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <Link
                            href={`${paths.dashboard.procurement.directPurchase.create}?edit=${dp.id}`}
                          >
                            <button
                              className="rounded p-1.5 hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
                              title="Edit"
