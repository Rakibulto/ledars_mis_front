...
import { getDeferredExpenseById } from './mock-data';
import { enrichDeferredExpense } from './use-deferred-expenses-api';
      ...
      <CardContent>
        ...
        <Stack spacing={2}>
          ...
          <Alert severity={catchUpAmount > 0 ? 'warning' : 'success'}>
            {catchUpAmount > 0
              ? `Amortization is behind plan by ${formatCurrency(catchUpAmount)}.`
...
export default function DeferredExpenseDetail({ recordId }) {
  const isNumeric = !Number.isNaN(Number(recordId));
  const { data: rawRecord } = useSWR(
    isNumeric ? endpoints.accounting.deferred_expense_by_id(recordId) : null,
    fetcher
  );
  const [record, setRecord] = useState(
    isNumeric ? null : (getDeferredExpenseById(recordId) ?? null)
  );
  useEffect(() => {
    if (rawRecord) setRecord(enrichDeferredExpense(rawRecord));
  }, [rawRecord]);
  ...
  if (!record) {
    return (
      <TransactionRecordNotFound
        ...
        backHref={paths.dashboard.accountingFinance.transactions.deferredExpenses}
      />
    );
  }
  ...
  return (
    <>
      ...
      <TransactionDetailShell
        title="Deferred Expense Detail"
        subtitle="Amortization schedule for prepaid operating expenditure and monthly recognition control."
        ...
        summary={[
          { label: 'Deferred value', value: formatCurrency(record.total_amount) },
        ...
        sections={[
          ...
          {
            title: 'Amortization Control Plan',
            items: [
              ...
              { label: 'Catch-up required', value: formatCurrency(catchUpAmount) },
              {
                ...
                value:
                  catchUpAmount > 0
                    ? 'Amortization is behind the planned release profile.'
                    : 'Amortization is aligned with the planned release profile.',
                fullWidth: true,
              },
            ],
          },
        ...
        sidebar={[
          {
            title: 'Accrual Control',
            items: [
              {
                primary: 'Schedule status',
                secondary: 'Recognition completion state',
                meta: record.status.replace(/_/g, ' '),
              },
              {
                primary: 'Remaining balance',
                secondary: 'Unrecognized prepaid expense',
                meta: formatCurrency(remaining),
              },
              {
                primary: 'Catch-up gap',
                secondary: 'Amortization still needed to align with elapsed periods',
        ...
        controlChecks={[
          {
            ...
            description: 'Schedule progress toward full amortization',
            status: record.status === 'done' ? 'success' : 'warning',
            value: progress,
          },
          {
            label: 'Balance remaining',
            description: 'Outstanding prepaid expense yet to recognize',
            status: remaining > 0 ? 'info' : 'success',
            value: formatCurrency(remaining),
          },
          {
            label: 'Schedule alignment',
            description: 'Comparison of expected amortization versus actual recognized expense',
          ...
          },
          {
            ...
            description:
              catchUpAmount > 0
                ...
                : 'Amortization aligned with elapsed periods',
