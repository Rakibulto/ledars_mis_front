...
import {
  exportPolicyConfigCsv,
  exportPolicyConfigJson,
  exportPolicyConfigExcel,
  buildPolicyConfigExportConfig,
} from './policy-config-export';
...
export function PolicyConfigToolbar({ printTitle = 'Policy Configuration', printContent = null }) {
  const pathname = usePathname();
  const workspace = usePolicyConfigWorkspace();
  const exportConfig = buildPolicyConfigExportConfig(workspace);
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
          <Button
            ...
            onClick={() =>
              runAction(
                'Export Policy CSV',
                () => exportPolicyConfigCsv('configuration-policy-controls', exportConfig),
                'Policy CSV exported'
              )
            }
          ...
          <Button
            ...
            onClick={() =>
              runAction(
                'Export Policy Excel',
                () => exportPolicyConfigExcel('configuration-policy-controls', exportConfig),
                'Policy workbook exported'
              )
            }
            disabled={pendingAction !== null}
          >
          ...
          <Button
            ...
            onClick={() =>
              runAction(
                'Export Policy JSON',
                () => exportPolicyConfigJson('configuration-policy-controls', exportConfig),
                'Policy JSON exported'
              )
            }
            disabled={pendingAction !== null}
          >
