...
export default function TravelExpenseList() {
  ...
  const apiUrl = useMemo(() => {
    ...
    return `${endpoints.travelExpense.list}?${params.toString()}`;
  ...
  const handleDelete = useCallback(async () => {
    if (!deleteId) return;
    try {
      await deleteRequest(endpoints.travelExpense.byId(deleteId));
  ...
  return (
    <DashboardContent>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
        <Typography variant="h4">Travel Expense</Typography>
        <Button
          variant="contained"
          startIcon={<Iconify icon="solar:add-circle-bold-duotone" />}
          onClick={() =>
            router.push('/dashboard/accounting-finance/transactions/travel-expence/create')
          }
        >
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={TABLE_HEAD.length} align="center">
                    <Typography variant="body2" color="text.secondary" sx={{ py: 4 }}>
                      No travel expense records found.
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
                data.map((row, index) => (
                  <TableRow
                    ...
                    onClick={() =>
                      router.push(
                        `/dashboard/accounting-finance/transactions/travel-expence/${row.id}`
                      )
                    }
                  >
                          ...
                          <Tooltip title="Edit">
                            <IconButton
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                router.push(
                                  `/dashboard/accounting-finance/transactions/travel-expence/create?editId=${row.id}`
                                );
                              }}
                            >
