...
export function EditAdvanceView() {
  ...
  useEffect(() => {
    ...
    // Match employee by ID first, then fall back to username match
    const matchedEmployee =
      (advance.from_employee_id
        ? employees.find((e) => e.id === advance.from_employee_id)
        : null) ||
      (advance.from_employee_name
        ? employees.find((e) => e.username === advance.from_employee_name)
        : null) ||
      null;

    // Match project by ID first (needs backend write_only fix), then by title
    const matchedProject =
      (advance.project_id_field ? projects.find((p) => p.id === advance.project_id_field) : null) ||
      (advance.project_title ? projects.find((p) => p.title === advance.project_title) : null) ||
      null;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ── KEY FIX: always append from_employee_id so PATCH never skips it ──
      // If null/unselected, send empty string — DRF's allow_null=True accepts it
      formData.append('from_employee_id', fromEmployee ? fromEmployee.id : '');
      ...
      localStorage.setItem('advance_updated', Date.now().toString());
