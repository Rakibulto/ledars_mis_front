...
import { createAdvance } from 'src/actions/advances';
...
export function CreateAdvanceView() {
  ...
  const { data: meData } = useGetRequest(endpoints.auth.me);
  ...
  // ── Default "To" employee to the logged-in user ──────────────────────────
  useEffect(() => {
    if (!fromEmployee && meData?.id && employees.length) {
      const currentUser = employees.find((e) => e.id === meData.id);
      if (currentUser) setFromEmployee(currentUser);
    }
  }, [meData, employees, fromEmployee]);
      ...
      <Stack direction="row" alignItems="center" spacing={1.5} sx={{ mb: 3 }}>
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Submit a new advance request under expense management
          </Typography>
        </Box>
      </Stack>
        ...
        >
          ...
          <Box sx={{ p: 3 }}>
            <Grid container spacing={2.5}>
              ...
              {/* Bank Name & Cheque No — conditional */}
              ...
              {/* Amount in Words — read only, auto-computed */}
              <Grid size={{ xs: 12 }}>
                <TextField
                  size="small"
