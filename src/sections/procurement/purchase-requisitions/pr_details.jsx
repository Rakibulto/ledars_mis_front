...
import AddItemsForRequisitionModal from './add-item-for-requistion-modal';
...
export default function PRDetails() {
  ...
  } = useGetRequest(`${endpoints.procurement.purchase_requisitions}${prId}/`);

  const prData = prDetailsData;
  ...
  const handleEdit = () => {
    router.push(`/dashboard/procurement/purchase-requisitions/add-pr/?edit_pr=${prData?.id}`);
  };
  const handleAddItem = () => {
    router.push(
      `/dashboard/procurement/purchase-requisitions/add-pr/?edit_pr=${prData?.id}&add_items=true`
    );
  };

  const handleDelete = async () => {
    try {
      await deleteData(`${endpoints.procurement.purchase_requisitions}${prId}/`);
  ...
  const handleSaveItem = async () => {
    ...
    await patchData(
      `${endpoints.procurement.items_for_requisition}${selectedItem?.id}/`,
      selectedItem
    );

    mutate(`${endpoints.procurement.purchase_requisitions}${prId}/`);
    mutate(`${endpoints.procurement.items_for_requisition}${selectedItem?.id}/`);
  ...
  const confirmDeleteItem = async () => {
    ...
    await deleteData(`${endpoints.procurement.items_for_requisition}${selectedItem?.id}/`);
    mutate(`${endpoints.procurement.purchase_requisitions}${prId}/`);
  ...
  const statusColor = getStatusColor(prData?.status);
  ...
  return (
    <Container maxWidth="2xl" sx={{ py: { xs: 2, sm: 3, md: 4 } }}>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            ...
            >
              <Stack spacing={0.75}>
                ...
                <Typography variant="body1" fontWeight={700}>
                  {prData?.created_by?.employee_name || '--'}
                </Typography>
              </Stack>
            </Paper>
          </Grid>
          ...
          <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            ...
            >
              <Stack spacing={0.75}>
                ...
                <Typography variant="body1" fontWeight={700}>
                  {new Date(prData?.created_at)?.toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'short',
                    day: 'numeric',
                  })}
            ...
            >
              <Stack spacing={0.75}>
                ...
                <Typography variant="h6" fontWeight={800} color="primary.darker">
                  ${prData?.estimated_amount?.toLocaleString()}
                </Typography>
              </Stack>
            </Paper>
          </Grid>
      ...
      >
        ...
        <TableContainer sx={{ overflow: 'auto' }}>
          <Table sx={{ minWidth: { xs: 800, md: 1000 } }}>
            ...
            <TableBody>
              {prData?.pr_items?.map((item, index) => {
              ...
              <TableRow align="right">
                <TableCell align="right" colSpan={8} sx={{ marginRight: 6 }}>
                  <Typography variant="body2" fontWeight={800}>
                    {`Total Price : $${prData?.estimated_amount?.toLocaleString()}`}
                  </Typography>
                </TableCell>
              </TableRow>
            </TableFooter>
      ...
      <ConfirmDialog
        ...
        content={`Are you sure you want to delete ${prData?.pr_number}? This action cannot be undone.`}
        action={
          <Button
            variant="contained"
            color="error"
