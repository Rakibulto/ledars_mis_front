...
function OfficeRow({ row, serial, onView, onEdit, onDelete, checkingStock }) {
  return (
    <TableRow
      hover
...
function OfficeFormDialog({ open, editing, form, onChange, onClose, onSubmit, submitting }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontWeight: 800 }}>{editing ? 'Edit Office' : 'Add Office'}</DialogTitle>
      <Divider />
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12 }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              />
              <Typography variant="caption" color="text.secondary">
                Read-only — always sent as &quot;office&quot;
              </Typography>
            </Stack>
          </Grid>
          ...
          {/* Head of Office — office/warehouse dropdown */}
          <Grid size={{ xs: 12 }}>
            <Autocomplete
              fullWidth
              size="small"
...
export default function OfficeSettingsMain() {
  ...
  const summaryUrl = useMemo(() => `${EP.office_management}?type=office&pagination=false`, []);
  ...
  return (
    <Box sx={{ p: 3, minHeight: '100vh', bgcolor: 'background.default' }}>
      <Stack spacing={3}>
        ...
        >
          <Box>
            <Stack direction="row" spacing={1.5} alignItems="center" mb={0.5}>
              ...
              <Box>
                ...
                <Typography variant="caption" color="text.secondary">
                  Manage office locations — staff, budgets, operational status, and details
                </Typography>
              </Box>
            </Stack>
          </Box>
          <Button
            ...
            sx={{
              ...
              background: primaryGradient,
              '&:hover': {
                background: `linear-gradient(135deg, ${theme.palette.secondary.dark}, ${theme.palette.warning.dark})`,
              },
            }}
          >
            Add Office
        ...
        >
          {listError ? (
            'Failed to load offices. Please check the backend or refresh.'
          ) : (
            <>
              Data source: <strong>/api/office_management/?type=office</strong> — with live search,
              status and division filters.
            </>
          )}
        </Alert>
        ...
        <Card sx={panelSx}>
                ...
                {listLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={9} align="center" sx={{ py: 6 }}>
                      <Stack alignItems="center" spacing={1}>
                        ...
                        />
                        <Typography variant="body2" color="text.secondary">
                          No office records match the current filters.
                        </Typography>
                      </Stack>
                    </TableCell>
                  </TableRow>
                  ...
                  rows.map((row, index) => (
                    <OfficeRow
                      ...
                      onView={(id) =>
                        router.push(paths.dashboard.storeInventory.officeSettingsDetail(id))
                      }
                      onEdit={openEdit}
          ...
          >
            <Typography variant="caption" color="text.secondary">
              {rawList?.count != null
                ? `Showing ${rows.length ? page * (rawList?.page_size || 10) + 1 : 0}–${page * (rawList?.page_size || 10) + rows.length} of ${rawList.count} office${rawList.count !== 1 ? 's' : ''}`
      ...
      <ConfirmDialog
        ...
        content={
          <Typography>
            Delete <strong>{deleteTarget?.name || 'this office'}</strong>? This action cannot be
            undone.
          </Typography>
        }
        action={
      ...
      <ConfirmDialog
        ...
        content={
          <Typography>
            <strong>{stockBlockTarget?.name || 'This office'}</strong> cannot be deleted because
