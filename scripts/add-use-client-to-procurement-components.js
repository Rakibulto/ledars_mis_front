...
function ensureUseClientDirective(code) {
...
async function main() {
  ...
  const targetDir = path.join(repoRoot, 'src', 'tailwind_components');
  ...
  for (const tsxFile of tsxFiles) {
    try {
      ...
      let output = transpileTsxToJsx(source, path.basename(tsxFile));

      // Ensure "use client"
      output = ensureUseClientDirective(output);
      ...
      console.error(`Error processing ${tsxFile}:`, err.message);
    }
  }

  console.log(`\nDone. Total converted: ${converted}`);
}
